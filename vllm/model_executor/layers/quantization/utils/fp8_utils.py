...
def w8a8_triton_block_scaled_mm(
    ...
    # checkpoints store block scales in exponent-only E8M0 format, so decode
    # them to fp32 before launching the kernel on every backend.
    if As.dtype in (torch.float8_e8m0fnu, torch.uint8):
